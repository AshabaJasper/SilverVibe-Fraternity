    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          ...
          <p className="text-[#64748b] text-sm">Generate, download, and schedule reports</p>
        </div>
      </div>
      ...
      {activeTab === "catalog" && (
        <div className="space-y-4">
          {REPORT_CATEGORIES.map((cat) => (
            <div key={cat.title} className="bg-white rounded-xl border border-[#e2e8f0] overflow-hidden">
              <button
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <h3 className="font-semibold text-[#1A1A1A] text-sm">{cat.title}</h3>
                    <p className="text-xs text-[#64748b]">{cat.reports.length} reports</p>
                  </div>
                </div>
                <ChevronDown className={`w-4 h-4 text-[#64748b] transition-transform ${expandedCategory === cat.title ? "rotate-180" : ""}`} />
              </button>
              {expandedCategory === cat.title && (
                <div className="border-t border-[#e2e8f0]">
                  {cat.reports.map((r) => (
                    <div key={r.name} className="px-5 py-3 flex items-center justify-between border-b border-[#e2e8f0] last:border-0 hover:bg-[#f8f9fa]">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-[#f8f9fa] border-b border-[#e2e8f0]">
                  <th className="text-left px-4 py-3 text-xs font-semibold text-[#64748b]">Report</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-[#64748b]">Category</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-[#64748b]">Generated By</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-[#64748b]">Date</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-[#64748b]">Format</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-[#64748b]">Size</th>
                  ...
                  <tr key={i} className="border-b border-[#e2e8f0] hover:bg-[#f8f9fa]">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-medium text-[#1A1A1A]">{r.name}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-xs text-[#64748b]">{r.type}</td>
                    <td className="px-4 py-3 text-xs text-[#64748b]">{r.generatedBy}</td>
                    <td className="px-4 py-3 text-xs text-[#64748b]">{r.date}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                        r.format === "PDF" ? "bg-[#ef4444]/10 text-[#ef4444]" : "bg-[#22c55e]/10 text-[#22c55e]"
                      }`}>{r.format}</span>
                    </td>
                    <td className="px-4 py-3 text-xs text-[#64748b]">{r.size}</td>
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <button className="p-1.5 rounded-lg hover:bg-[#f1f5f9]"><Download className="w-3.5 h-3.5 text-[#64748b]" /></button>
                        <button className="p-1.5 rounded-lg hover:bg-[#f1f5f9]"><Printer className="w-3.5 h-3.5 text-[#64748b]" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {activeTab === "scheduled" && (
        <div className="space-y-4">
          {[
            ...
            <div key={s.name} className="bg-white rounded-xl p-5 border border-[#e2e8f0] flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="font-semibold text-[#1A1A1A] text-sm">{s.name}</h3>
              ...
              <div className="flex gap-2">
                ...
                <button className="px-3 py-1.5 border border-[#e2e8f0] rounded-lg text-xs font-medium text-[#ef4444] hover:bg-[#ef4444]/5 transition-colors">
                  Disable
                </button>
              </div>
            </div>
