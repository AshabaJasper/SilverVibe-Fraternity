  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
        <div>
          <h1 className="text-2xl font-bold text-[#1A1A1A]">Loans</h1>
          <p className="text-[#64748b] text-sm">Manage your loan accounts and applications</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="glass-card rounded-2xl p-5 animate-fade-in">
          ...
          <p className="text-2xl font-bold text-[#1A1A1A]">{formatCurrency(loan.total_outstanding)}</p>
        </div>
        <div className="glass-card rounded-2xl p-5 animate-fade-in stagger-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-[#64748b]">Repaid</span>
          </div>
        ...
        <div className="glass-card rounded-2xl p-5 animate-fade-in stagger-2">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-[#64748b]">Next Payment</span>
          </div>
          <p className="text-2xl font-bold text-[#f59e0b]">{formatCurrency(loan.next_payment_amount)}</p>
          <p className="text-xs text-[#64748b] mt-1">Due: {loan.next_payment_date}</p>
        </div>
        <div className="glass-card rounded-2xl p-5 animate-fade-in stagger-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-[#64748b]">Eligibility</span>
          </div>
          <p className="text-2xl font-bold text-[#a855f7]">{formatCurrency(maxEligible)}</p>
          <p className="text-xs text-[#64748b] mt-1">Max borrowable</p>
        </div>
      </div>
      ...
      <div className="glass-card rounded-2xl p-6 animate-fade-in stagger-2">
        <div className="flex items-start justify-between mb-6">
          <div>
            <h3 className="font-semibold text-[#1A1A1A] text-lg">{loan.product}</h3>
            <p className="text-sm text-[#64748b]">{loan.loan_number}</p>
          </div>
        ...
        <div className="mb-6">
          <div className="flex justify-between text-sm mb-2">
            <span className="text-[#64748b]">Repayment Progress</span>
            <span className="font-semibold text-[#1A1A1A]">{loan.repayment_progress}%</span>
          </div>
        ...
        {/* Loan Details Grid */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-[#f1f5f9] rounded-xl">
          {[
      ...
      <div className="glass-card rounded-2xl p-6 animate-fade-in stagger-3">
        <h3 className="font-semibold text-[#1A1A1A] mb-4">Repayment Breakdown</h3>
        <div className="h-64">
      ...
      <div className="glass-card rounded-2xl p-6">
        ...
        >
          ...
          {showSchedule ? <ChevronUp className="w-5 h-5 text-[#64748b]" /> : <ChevronDown className="w-5 h-5 text-[#64748b]" />}
        </button>
        {showSchedule && (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#e2e8f0]">
                  ...
                  <th className="text-left py-3 px-2 text-[#64748b] font-medium">Due Date</th>
                  <th className="text-right py-3 px-2 text-[#64748b] font-medium">Principal</th>
                  <th className="text-right py-3 px-2 text-[#64748b] font-medium">Interest</th>
                  <th className="text-right py-3 px-2 text-[#64748b] font-medium">Total</th>
                  <th className="text-center py-3 px-2 text-[#64748b] font-medium">Status</th>
                </tr>
              </thead>
              <tbody>
                {LOAN_SCHEDULE.map((s) => (
                  <tr key={s.installment} className={`border-b border-[#e2e8f0]/50 ${s.status === "Paid" ? "bg-[#22c55e]/5" : ""}`}>
                    ...
                    <td className="py-3 px-2 text-right text-[#64748b]">{formatCurrency(s.interest)}</td>
      ...
      <div className="glass-card rounded-2xl p-6">
        ...
        >
          ...
          {showCalculator ? <ChevronUp className="w-5 h-5 text-[#64748b]" /> : <ChevronDown className="w-5 h-5 text-[#64748b]" />}
        </button>
        {showCalculator && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                <label className="text-sm text-[#64748b] mb-1 block">Loan Amount (UGX)</label>
                <input type="number" value={calcAmount} onChange={(e) => setCalcAmount(e.target.value)}
                  className="w-full border border-[#e2e8f0] rounded-xl px-4 py-3 text-[#1A1A1A] focus:outline-none focus:border-[#4A90D9] transition-colors" />
              </div>
              <div>
                <label className="text-sm text-[#64748b] mb-1 block">Period (Months)</label>
                <input type="number" value={calcMonths} onChange={(e) => setCalcMonths(e.target.value)}
                  className="w-full border border-[#e2e8f0] rounded-xl px-4 py-3 text-[#1A1A1A] focus:outline-none focus:border-[#4A90D9] transition-colors" />
              </div>
              <div>
                <label className="text-sm text-[#64748b] mb-1 block">Interest Rate (% p.a.)</label>
                <input type="number" value={calcRate} onChange={(e) => setCalcRate(e.target.value)}
                  className="w-full border border-[#e2e8f0] rounded-xl px-4 py-3 text-[#1A1A1A] focus:outline-none focus:border-[#4A90D9] transition-colors" />
              </div>
            </div>
            <div className="bg-[#f1f5f9] rounded-xl p-6 flex flex-col items-center justify-center">
              <p className="text-sm text-[#64748b] mb-2">Estimated Monthly Payment</p>
              <p className="text-3xl font-bold text-[#4A90D9]">{formatCurrency(Math.round(calcMonthly()))}</p>
              <div className="mt-4 grid grid-cols-2 gap-4 w-full">
                <div className="text-center">
                  <p className="text-xs text-[#64748b]">Total Repayment</p>
                  <p className="text-sm font-semibold text-[#1A1A1A]">
                    {formatCurrency(Math.round(calcMonthly() * (parseInt(calcMonths) || 1)))}
                  </p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-[#64748b]">Total Interest</p>
