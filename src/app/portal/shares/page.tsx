...
export default function SharesPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div>
        <h1 className="text-2xl font-bold dm-text">Shares & Investments</h1>
        <p className="dm-text-secondary text-sm">Your share capital and investment units</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="glass-card rounded-2xl p-5 animate-fade-in">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm dm-text-secondary">Share Capital</span>
          </div>
          <p className="text-2xl font-bold dm-text">{formatCurrency(MEMBER_SHARES.total_value)}</p>
          <p className="text-xs dm-text-secondary mt-1">{MEMBER_SHARES.num_shares} shares @ {formatCurrency(MEMBER_SHARES.share_value)} each</p>
        </div>
        <div className="glass-card rounded-2xl p-5 animate-fade-in stagger-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm dm-text-secondary">Investment Units</span>
          </div>
          <p className="text-2xl font-bold text-[#a855f7]">{formatCurrency(MEMBER_IU.current_value)}</p>
          <p className="text-xs dm-text-secondary mt-1">{MEMBER_IU.unit_balance} units</p>
        </div>
        <div className="glass-card rounded-2xl p-5 animate-fade-in stagger-2">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm dm-text-secondary">Total Dividends</span>
          </div>
        ...
        <div className="glass-card rounded-2xl p-5 animate-fade-in stagger-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm dm-text-secondary">IU Returns</span>
          </div>
          <p className="text-2xl font-bold text-[#4A90D9]">{formatCurrency(MEMBER_IU.accrued_returns)}</p>
          <p className="text-xs dm-text-secondary mt-1">15% return on investment</p>
        </div>
      </div>
        ...
        <div className="glass-card rounded-2xl p-6 animate-fade-in stagger-2">
          ...
          <div className="space-y-3">
            {MEMBER_SHARES.certificates.map((cert) => (
              <div key={cert.number} className="flex items-center justify-between p-4 dm-surface-hover rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium dm-text">{cert.number}</p>
                    <p className="text-xs dm-text-secondary">{cert.shares} shares · {cert.date}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-sm font-semibold dm-text">{formatCurrency(cert.amount)}</span>
                  <button className="p-2 rounded-lg hover:bg-[#e2e8f0] transition-colors">
                    <Download className="w-4 h-4 dm-text-secondary" />
                  </button>
                </div>
              </div>
            ))}
        ...
        <div className="glass-card rounded-2xl p-6 animate-fade-in stagger-3">
          ...
          <div className="space-y-2">
            {MEMBER_SHARES.dividends.map((d) => (
              <div key={d.year} className="flex items-center justify-between text-sm p-3 dm-surface-hover rounded-xl">
                <div className="flex items-center gap-2">
                  <Calendar className="w-4 h-4 dm-text-secondary" />
                  <span className="dm-text font-medium">{d.year}</span>
                  <span className="text-xs dm-text-secondary">@ {d.rate}</span>
                </div>
      ...
      <div className="glass-card rounded-2xl p-6 animate-fade-in stagger-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-gradient-to-r from-[#a855f7]/5 to-[#4A90D9]/5 rounded-xl border border-[#a855f7]/10">
          <div>
            <p className="text-xs dm-text-secondary">Units Held</p>
            <p className="text-lg font-bold dm-text">{MEMBER_IU.unit_balance}</p>
          </div>
          <div>
            <p className="text-xs dm-text-secondary">Total Invested</p>
            <p className="text-lg font-bold dm-text">{formatCurrency(MEMBER_IU.total_invested)}</p>
          </div>
          <div>
            <p className="text-xs dm-text-secondary">Accrued Returns</p>
            <p className="text-lg font-bold text-[#22c55e]">{formatCurrency(MEMBER_IU.accrued_returns)}</p>
          </div>
          <div>
            <p className="text-xs dm-text-secondary">Current Value</p>
            <p className="text-lg font-bold text-[#a855f7]">{formatCurrency(MEMBER_IU.current_value)}</p>
          </div>
        </div>
        <p className="text-xs dm-text-secondary mt-3">
